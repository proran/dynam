function [otim, ofrc, ofrr] = post_res_read_v01_comb(varargin)
...
for el = elout
    ...
    if k <4
    plot(tim,frr,fmt{k},'LineWidth',2.5); hold on;
    lgnd{k} = ['e',num2str(el)];
    lgnd{k+3} = '';
    end
...
title(['Resultant Belt Force'],'Units','normalized','Position',[0.5,1,0],'FontSize',20);
...
post_save_tif([out,filesep,job,'_combined.tif']);
hgsave([out,filesep,job,'_combined.fig']);